...
use aegis_core::{
    AegisCore, Config, 
    extract::{Extractor, ExtractionResult, ExtractionError},
    archive::{ComplianceLevel, ComplianceProfile},
    resource::Resource,
    patch::{PatchRecipe, PatchApplier},
};
...
impl PyAegisCore {
    ...
    fn load_compliance_profiles(&mut self, profiles_dir: &str) -> PyResult<()> {
        ...
        self.inner.load_compliance_profiles(&path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))
    }
    ...
    fn system_info(&self) -> PyResult<PyDict> {
...
impl PyExtractor {
    ...
    fn extract_from_file(&mut self, source_path: &str, output_dir: &str) -> PyResult<PyExtractionResult> {
        let source = PathBuf::from(source_path);
        let output = PathBuf::from(output_dir);
        
        let result = self.inner.extract_from_file(&source, &output)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
    ...
    fn extract_batch(&mut self, source_paths: Vec<&str>, output_dir: &str) -> PyResult<Vec<PyExtractionResult>> {
        ...
        let results = self.inner.extract_batch(source_refs, &output)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        
        Ok(results.into_iter().map(|r| PyExtractionResult { inner: r }).collect())
    }
}
...
impl PyExtractionResult {
    ...
    #[getter]
    fn compliance_info(&self) -> PyResult<PyDict> {
        Python::with_gil(|py| {
            let dict = PyDict::new(py);
            dict.set_item("is_compliant", self.inner.compliance_info.is_compliant)?;
            dict.set_item("risk_level", compliance_level_to_string(self.inner.compliance_info.risk_level))?;
            dict.set_item("warnings", &self.inner.compliance_info.warnings)?;
            dict.set_item("recommendations", &self.inner.compliance_info.recommendations)?;
            Ok(dict.into())
        })
    }
    ...
    #[getter]
    fn metrics(&self) -> PyResult<PyDict> {
        Python::with_gil(|py| {
            let dict = PyDict::new(py);
    ...
    fn get_resources(&self) -> PyResult<Vec<PyDict>> {
        Python::with_gil(|py| {
...
impl PyPatchRecipe {
    ...
    fn save_to_file(&self, path: &str) -> PyResult<()> {
        self.inner.save_to_file(&PathBuf::from(path))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))
    }
    
    /// Get recipe summary
    fn summary(&self) -> PyResult<PyDict> {
...
impl PyPatchApplier {
    ...
    fn apply(&self, source_file: &str, output_dir: &str) -> PyResult<PyDict> {
        let result = self.inner.apply(&PathBuf::from(source_file), &PathBuf::from(output_dir))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        ...
        Python::with_gil(|py| {
            ...
            // Applied files
            let files: PyResult<Vec<PyDict>> = result.applied_files.iter().map(|f| {
...
#[pyfunction]
fn extract_assets(source_path: &str, output_dir: &str, compliance_profiles_dir: Option<&str>) -> PyResult<PyExtractionResult> {
    ...
    extractor.extract_from_file(source_path, output_dir)
}

#[pyfunction]
fn apply_patch_recipe(recipe_path: &str, source_file: &str, output_dir: &str) -> PyResult<PyDict> {
    let applier = PyPatchApplier::from_file(recipe_path)?;
    applier.apply(source_file, output_dir)
}
...
fn aegis_assets(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add("__doc__", "Compliance-first platform for game asset extraction, preservation, and creative workflows")?;
