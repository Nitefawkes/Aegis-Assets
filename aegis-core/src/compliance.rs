...
impl ComplianceChecker {
    ...
    pub fn load_profiles(&mut self, profiles_dir: &Path) -> Result<usize> {
        debug!("Loading compliance profiles from: {}", profiles_dir.display());
        
        if !profiles_dir.exists() {
            warn!("Compliance profiles directory does not exist: {}", profiles_dir.display());
            return Ok(0);
        }
    ...
    fn get_profile_for_game(&self, game_id: &str) -> &ComplianceProfile {
        ...
        DEFAULT_PROFILE.get_or_init(|| ComplianceProfile {
            ...
            enterprise_warning: Some(
                "Unknown publisher. Compliance status unclear.".to_string()
            ),
            mod_policy_url: None,
...
fn calculate_risk_score(allowed: usize, warned: usize, high_risk: usize, blocked: usize) -> f64 {
    ...
    let risk_weighted = (warned as f64 * 0.25) + 
                       (high_risk as f64 * 0.75) + 
                       (blocked as f64 * 1.0);
...
fn generate_recommendations(issues: &[ComplianceIssue]) -> Vec<String> {
    ...
    let high_risk_count = issues.iter()
    ...
    if high_risk_count > 0 {
        ...
        recommendations.push(
            "Consider enabling strict compliance mode to automatically block high-risk operations.".to_string()
        );
    }
    
    recommendations.push(
        "Regularly update compliance profiles to reflect changes in publisher policies.".to_string()
    );
    
    recommendations.push(
        "Maintain audit logs of all extraction activities for legal compliance.".to_string()
    );
